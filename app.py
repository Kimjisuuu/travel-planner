...
COUNTRY_DATA = {
    ...
    "이탈리아": {"도시": ["로마", "밀라노", "피렌체", "베네치아"], "언어": "it"}, # 이탈리아 추가!
    "기타(직접 검색)": {"도시": [], "언어": "en"}
}
...
with st.expander("🌤️ 실시간 날씨 & 🗣️ 빠른 번역 (여기를 눌러 여닫으세요)", expanded=True):
    ...
    with col_w2:
        if selected_country == "기타(직접 검색)":
            selected_city = st.text_input("도시명 직접 입력 (예: 피렌체, 프라하)")
        else:
    ...
    if selected_city:
        if selected_country == "기타(직접 검색)":
            search_query_weather = selected_city # 국가명 생략하고 도시명만 검색!
        else:
            search_query_weather = f"{selected_country} {selected_city}"
            
        locs = search_places(search_query_weather)
        if locs:
            ...
            st.caption("해당 지역의 위치 정보를 찾을 수 없습니다. 영문으로 다시 검색해 보세요.")
...
with tab2:
    ...
    with col_exp_in:
        ...
        # 💡 [수정됨] 콤마 표시 및 한글 텍스트 동시 출력
        if e_amount > 0:
            st.success(f"💸 입력 확인: **{e_amount:,.0f}** 원  👉  **({num_to_kr(e_amount)})**")
